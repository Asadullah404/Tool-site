...
const FinancialCalculatorPage: React.FC = () => {
  ...
  const validTypes: CalculationType[] = [
    'EMI',
    'ProfitLoss',
    'SimpleInterest',
    'CompoundInterest',
  ...
  useEffect(() => {
    if (type && validTypes.includes(type as CalculationType)) {
      setCalculationType(type as CalculationType);
      setInputs({});
      setResult('');
    }
  }, [type]);
      ...
      <div className="flex gap-4 mb-6 flex-wrap justify-center">
        {validTypes.map((type) => (
          <Button
            ...
            onClick={() => navigate(`/financial-calculator/${type}`)}
          >
            {type}
          </Button>
        ))}
