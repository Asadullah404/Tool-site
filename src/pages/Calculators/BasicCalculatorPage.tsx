...
//   // Safe evaluation using Algebrite
...
//     } else if (value === '=') {
//       try {
//         // Replace symbols for Algebrite
//         let expression = input.replace(/√/g, 'sqrt'); // √ → sqrt
//         expression = expression.replace(/%/g, '/100'); // % → divide by 100
//         // Evaluate using Algebrite
//         const res = Algebrite.run(expression);
...
//   // Split buttons into rows for 4-column grid
  ...
  const handleButtonClick = async (value: string) => {
    if (value === 'C') {
      ...
      try {
        ...
        let expression = input.replace(/√/g, 'sqrt').replace(/%/g, '/100');
        const res = Algebrite.run(expression);
        setResult(res.toString());
      } catch {
        setResult('Error');
      ...
      <div className="grid grid-cols-4 gap-4 max-w-md w-full mb-4">
        {buttonRows.flat().map((btn, idx) => (
          <Button
            key={btn+idx}
            className={`px-4 py-4 rounded-xl font-bold text-lg ${
              btn === 'C' ? 'bg-red-500 text-white shadow-lg hover:scale-105 transform transition' 
                        : 'bg-card text-foreground shadow hover:scale-105 transform transition'
            }`}
            onClick={() => handleButtonClick(btn)}
          >
