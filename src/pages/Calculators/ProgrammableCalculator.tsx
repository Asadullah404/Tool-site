...
const ProgrammableCalculatorPage: React.FC = () => {
  ...
  const handleCalculate = async ()=>{
    try{
      ...
      let res = '';
      switch(mode){
        case 'Evaluate': res = Algebrite.run(expression).toString(); break;
        case 'Simplify': res = Algebrite.simplify(expression).toString(); break;
        case 'Factor': res = Algebrite.factor(expression).toString(); break;
        case 'Expand': res = Algebrite.expand(expression).toString(); break;
        case 'Derivative': res = Algebrite.derivative(expression,'x').toString(); break;
        case 'Integrate': res = Algebrite.integral(expression,'x').toString(); break;
        case 'Solve': res = Algebrite.run(`roots(${expression})`).toString(); break;
        default: res = 'Invalid mode';
      }
      setResult(res);
    }catch(err){
      console.error(err);
  ...
  return (
    <div className="min-h-screen flex flex-col items-center bg-muted/10 py-12 px-4">
      ...
      <Card className="w-full max-w-2xl p-6 mb-6 bg-gradient-to-br from-card/80 to-card/50 border border-white/20 shadow-2xl space-y-4">
        <label className="text-lg font-semibold text-foreground mb-2 block">Enter Expression:</label>
        <input
          type="text"
          value={expression}
          onChange={(e)=>setExpression(e.target.value)}
